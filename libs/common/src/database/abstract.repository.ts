...
export class AbstractRepository<TDocument extends AbstractDocument> {
    ...
    async findOne(filterQuery: FilterQuery<TDocument>): Promise<TDocument>{
        const document: TDocument = await this.model.findOne(filterQuery).lean<TDocument>(true)
        if (!document){
    ...
    async findOneAndUpdate(filterQuery: FilterQuery<TDocument>, update: UpdateQuery<TDocument>): Promise<TDocument>{
        const document: TDocument = await this.model.findOneAndUpdate(
            filterQuery, update, {new:true}
        ).lean<TDocument>(true)
        if (!document){
            this.logger.warn('Document was not found with filterQuery',filterQuery);
            throw new NotFoundException('Document was not found.')
